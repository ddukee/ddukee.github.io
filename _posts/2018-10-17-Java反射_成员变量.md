...
## 获取Field
Java的反射API提供了四个获取类中成员变量的方法。通过这些方法，可以获取类中定义的所有成员变量以及父类中定义的成员变量。
...
### getFields()
`getFields()`方法返回类或接口中定义的所有public成员变量以及从父类和接口中继承的所有public成员变量。如果对象是一个原始类型、void或者是数组类型，则返回长度为0的数组。数组中的成员变量的顺序是无序的。
...
### getField()
`getField()`方法支持通过成员变量名称获取声明的public成员变量和从父类或接口中继承的public成员变量。如果成员变量不存在或者成员变量不是声明为public的，则抛出 **NoSuchFieldException** 异常。对于数组类型，`getField()`方法获取不到`length`成员变量的值。

`getField()`方法在查询成员变量的时候，按照下面的流程进行搜索：

1. 搜索当前类中定义的所有public成员变量。
2. 搜索当前类的直接实现接口中定义的所有public成员变量，按照接口的定义顺序进行搜索。
3. 如果有父类，则在父类中执行 1~3 这三个步骤。
...
`getDeclaredFields()`方法获取类或接口中声明的所有成员变量，包括非public的成员变量。如果类或接口中未定义任何成员变量，则返回长度为0的数组。如果是一个原始类型、void类型或者是数组类型，则返回成员数组长度也为0。返回的数组中成员变量的位置是无序的。
...
`getDeclaredField()`方法支持通过成员变量的名称获取类或接口中定义的所有成员变量的Field对象，包括非public的成员变量。如果成员变量不存在，则抛出 **NoSuchFieldException** 异常。对于数组类型，不能通过`getDeclaredField()`获取 **length** 成员的Field对象。
...
### 解析修饰符
通过`getModifiers()`方法可以获取成员变量的修饰符。成员变量的修饰符包括：访问修饰符：**public** 、**protected** 、**private**；运行时行为相关的修饰符：**transient** 、**volatile**；静态修饰符：**static**；只读修饰符：**final** 等。
...
获取了成员变量的Field对象以后，就可以绕过setter方法，通过一些列`set*()`方法对成员变量进行赋值。对Java中的原始类型，**Field** 类中定义了一系列的`set*()`方法。
...
## 访问私有成员
如果成员变量是私有的，默认当在Field上调用get或者set方法时会抛出 **IllegalAccessException** 异常。
...
为了支持通过反射的方式访问私有成员，**Field** 类中提供了一个开放私有权限的方法：`setAccessible()`。通过调用`setAccessible(true)`将成员的访问权限设置为公共的，可以访问私有成员。如果`setAccessible()`的参数为false，则表示遵循Java的访问权限控制机制。
